...
需要 **VS Code 1.106 或更新版本**。执行 AI 任务前，请安装并登录其中一个受支持的 Agent CLI，Dext 使用该 CLI 的登录凭据。

Dext 默认位于副侧栏（VS Code 默认布局的右侧）。安装或更新后，未手动移动过的 Dext 会使用该默认位置；已手动调整的位置由 VS Code 保留。若仍显示在其他位置，可右键 Dext 标题，选择移动到副侧栏。
