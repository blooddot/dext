...
export function runDiagramProcess(command: string, args: readonly string[], cwd: string, signal?: AbortSignal): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(command, [...args], { cwd, shell: false, windowsHide: true, detached: process.platform !== "win32",
      env: { ...process.env, ELECTRON_RUN_AS_NODE: "1", ARCHIFY_UPDATE_CHECK_DISABLED: "1", PYTHONDONTWRITEBYTECODE: "1" } });
    ...
    const timer = setTimeout(() => stop("Diagram renderer exceeded 30 seconds."), 30_000);
    ...
    child.on("close", (code) => { cleanup(); if (failure) reject(failure); else if (code !== 0) reject(new Error((stderr || stdout || "Diagram renderer failed.").slice(0, 6000))); else resolve(stdout); });
  });
}
...
export function assertDiagramDocument(document: DiagramAdapterDocument, id: string, version: string, options?: DiagramAdapterRenderOptions): void {
  ...
  if (document.adapterId !== id || document.adapterVersion !== version) throw new Error("Incompatible diagram adapter document.");
}
...
export function diagramArtifact(document: DiagramAdapterDocument, format: DiagramAdapterArtifact["format"], content: string): DiagramAdapterArtifact {
  const mimeType = format === "html" ? "text/html" : format === "svg" ? "image/svg+xml" : format === "drawio" ? "application/xml" : "text/plain";
