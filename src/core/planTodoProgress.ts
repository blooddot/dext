...
export class PlanTodoProgress {
  ...
  consume(event: AgentStreamEvent): AgentStreamEvent[] {
    ...
    for (const match of previous.raw.matchAll(/<!-- dext-todo:([\s\S]*?)-->/g)) {
      ...
      if (typeof parsed !== "object" || parsed === null || !("updates" in parsed) || !Array.isArray(parsed.updates)) continue;
      const updates: Array<{ item: AgentTodoItem; status: AgentTodoItem["status"] }> = [];
      for (const value of parsed.updates as unknown[]) {
        if (typeof value !== "object" || value === null || !("id" in value) || !("status" in value)) { updates.length = 0; break; }
        const item = this.items.find((candidate) => candidate.id === value.id);
        if (!item || !["pending", "in_progress", "completed"].includes(String(value.status))) { updates.length = 0; break; }
