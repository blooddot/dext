...
2. **AI 生成** —— 所选项目 AI CLI 返回一份严格 JSON，包含 Project Intent 和零到多张图。证据、稳定 ID 引用和各类图所需的语义结构都会先对照本次受限输入校验，再进入保存阶段。Codex 与 Claude 通过各自的原生结构化输出通道（`--output-schema` / `--json-schema`）接收响应 schema，答案由提供方约束，而不只是写在提示词里；若某个 CLI 拒绝该 schema 或无法启动，本次尝试会回退到对话通道并说明原因。信封与一次有界修复由共享的 ax 预测器负责：严格契约解析与证据校验作为它的校验步骤运行，被拒绝的回答会带着这些诊断重新提问，契约未声明的多余键会在校验前丢弃，而不是让本可验证的生成失败。DeepSeek Harness 的 ACP 没有 schema 字段，因此始终走对话通道。
